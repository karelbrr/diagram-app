...
export function TrashDialog({ open, onOpenChange }: TrashDialogProps) {
  const { trashedShapes, restoreShape, permanentlyDeleteShape, emptyTrash } = useAppStore();
  ...
  const getShapeIcon = (type: string) => {
    switch (type) {
      ...
      case "polyline": return <Activity size={16} />;
                ...
                <div key={shape.id} className="flex items-center justify-between bg-zinc-900/50 p-3 rounded-lg border border-zinc-800">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-medium">{getShapeName(shape)}</span>
                      <span className="text-xs text-zinc-500 font-mono">ID: {shape.id.substring(0, 8)}</span>
                    </div>
                  </div>
